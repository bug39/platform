...
def py_to_uni_rsrc_request(
    ...
    def make_args(
        ...
        args_def: list[FunctionArgument] | None = None,
        new_defs: dict[DefnUID, DefUniMsg] = {},
        ...
        case CallMsg():
            ...
            if isinstance(fun_def, (FunctionDefUniMsg, MethodSignatureUniMsg)):
                ...
                if (
                    ...
                    and len(info.pos) >= required_args + 1
                    and (has_rest or len(info.pos) <= len(fun_def.payload.arguments) + 1)
                ):
            ...
            if defmsg.data.qname is not None and defmsg.data.name != defmsg.data.qname:
                ...
                if cls_uid is not None:
                    uni_def = toMethodSignature(
                        uni_def,
                        Membership(uid=cls_uid.uid, kind='instance'),
                    )
        case ObjectDataMsg():
...
def class_data_to_class_payload(
    ...
    if data.name == 'GenericAlias':
        return ClassPayload(name="GenericAlias", fields=[], methods=[])

    bases: list[RefUniMsg] = []
    methods: list[ClassMethod] = []
    fields: list[ClassField] = []
    ctor_args: list[FunctionArgument] | UnsetType = UNSET
    supplied_type_args: list[TypeArgument] | UnsetType = UNSET
    ...
    return ClassPayload(
        ...
        instance_of_generic=UNSET,
        bases=bases,
        methods=methods,
        fields=fields,
        supplied_type_args=supplied_type_args,
        ctor_args=ctor_args,
    )
