...
class Agent(LogBase):
    ...
    def __init__(
        ...
        *,
        ...
        mode : {'code', 'json'}
            The agent execution interaction mode.
            The mode 'code' allows invocations of the agent to communicate and use the scope
            (including tools attached to any MCP server specified by the mcp argument)
            through code, whereas 'json' only allows for classical tool calling without the richer logic enabled by having access to a programming language.
        model : str
